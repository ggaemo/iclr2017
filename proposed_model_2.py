...
class Beta_VAE_Discrim():
    def __init__(self, encoder_layer, decoder_layer, input_dim, fc_dim, z_dim,
                 ...
                 context_class_num, style_included, discrim_lambda, deterministic_c):
        ...
        self._build_latent(fc_dim, z_dim, num_partition, discriminator_layer,
                           context_class_num, style_included, deterministic_c)
    ...
    def _build_latent(self, fc_dim, z_dim, num_partition, discriminator_layer,
                      class_num, style_included, deterministic_c):

        with tf.variable_scope('latent'):

            self.z_m_c, self.z_log_sigma_sq_c, self.z_v_c = get_param(self.encoder_output,
                                                                      z_dim,
                                                                      deterministic_c)

            self.z_m_s, self.z_log_sigma_sq_s, self.z_v_s = get_param(self.encoder_output,
                                                                      z_dim, False)
            self.permutation_s = tf.placeholder(tf.int32, [None],
                                              name='permutation_s')
            self.permutation_c = tf.placeholder(tf.int32, [None],
                                                name='permutation_c')
            self.permutation_original = tf.placeholder(tf.int32, [None],
                                                name='permutation_original')

            self.partitioned_c = tf.dynamic_partition(self.z_v_c, self.permutation_original,
                                                         class_num)
            ...
            self.latent = tf.concat([self.shuffled_content, self.z_v_s], axis=1)

            if style_included:
                self.partitioned_s = tf.dynamic_partition(self.z_v_s,
                                                          self.permutation_original,
                                                          class_num)
                self.permutation_s_partitioned = tf.dynamic_partition(self.permutation_s,
                                                                      self.permutation_original,
                                                                      class_num)
                self.shuffled_style = tf.dynamic_stitch(self.permutation_s_partitioned,
                                                          self.partitioned_s)

                self.discrim_input = tf.concat([self.z_v_c, self.shuffled_style], axis=1)
            else:
                self.discrim_input = self.z_v_c
            ...
            if output_prob == 'bernoulli':
                self.recon_loss = tf.reduce_mean(tf.reduce_sum(
                    tf.nn.weighted_cross_entropy_with_logits(
                        targets=self.input_data, logits=self.logit, pos_weight=10), [1, 2,
                                                                                  3]))
    ...
    def _build_summary(self, z_dim, beta_s):
        ...
        summary_image_list.append(tf.summary.image('original', self.input_data,
                                                   max_outputs=1))
        summary_image_list.append(tf.summary.image('recon', self.recon, max_outputs=1))
